...
const MessageCatalog: FC<MessageCatalogProps> = ({
  ...
  return (
    <main className="grid justify-center pb-48 px-3 min-h-full">
      ...
      <section
        className="grid md:grid-cols-1 xl:grid-cols-2 w-full justify-between max-w-[1700px] mt-20 gap-x-[164px] gap-y-[80px] transition-all
        min-h-[40vh]"
      >
      ...
      {messagesLength && (
        <nav aria-label="Page navigation example transition-all">
          <ul className="-space-x-px text-base h-10 mt-20 flex justify-center gap-3 transition-all">
            <li>
              <button
                ...
                border-gray-300 rounded-lg ${
                  page === 1 ? "text-textPrimary" : "text-textSecondary"
                }`}
                onClick={(event) => {
            ...
            {Array.from(
              { length: Math.ceil(messagesLength / pagePerSize) },
              (_, index) => (
                <li key={index + 1}>
                  <button
                    className={`flex items-center justify-center px-4 h-10 ms-0 leading-tight text-gray-500 bg-white border
                    border-gray-300 rounded-lg  text-textSecondary transition-allq ${
                      page === index + 1 ? "bg-secondary" : ""
                    }`}
                    value={index + 1}
                    onClick={(event) => {
                      const button = event.target as HTMLButtonElement;
            ...
            <li>
              <button
                disabled={messages && messages?.length % 9 > 1}
                className={`flex items-center justify-center px-4 h-10 ms-0 leading-tight text-gray-500 bg-white border
                  border-gray-300 rounded-lg ${
                    messages && messages?.length % 8 > 1
                      ? "text-textPrimary"
                      : "text-textSecondary"
                  }`}
                onClick={(event) => {
